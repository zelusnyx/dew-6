...
class requestSolver():
    ...
    def __init__(self):
        self.var_map = dict()
        self.num_to_var = dict()
        self.ids = dict()

    # Adapting the version written to my standard to work for xir json

    def trySubsets(self, request, resources, resource_names = None):
        indexSet = set([])
        indexSet.update(range(0, len(resources)))

        solutions = []
        # For now, this works as long as sites don't have overlapping resources.
        # Really what we want is to find all combos that will help us out.
        for i in range(1, len(resources)+1):
            ...
            for combo in combos:
                ...
                subresourcenumbers = []
                for x in combo:
                    subresourcenumbers.append(x)
                    subresourcelist.append(resources[x])
                    if resource_names != None:
                        try:
                            subresourcenames.append(resource_names[x])
                        except:
                            pass
                
                if resource_names != None:
                    comboResult['site_combo'] = subresourcenames
                else:
                    comboResult['site_combo'] = subresourcenumbers
                
                names, solution = self.solve(request, subresourcelist)
                
                print(subresourcenames)
                if names != None:
                    print("SOLUTION")
                    comboResult['result'] = 'solution'
            ...
            if len(solutions) > 0:
                ...
                #else:	
                #    print("NO SOLUTION")
                #    comboResult['result'] = 'no solution'

            
        jsonResult = {}
        jsonResult['results'] = solutions
    ...
    def solve(self, request, resources, print_debug=False, print_vars=False, print_names=False, print_assignment=True, print_timing=True):
        start = time.time()
        clauses = self.generate_clauses(request, resources, print_debug)
        
        # It's going to take us too much time to solve this for a GUI demo,
        # so skip.
        if len(clauses) > 3000:
            print("Too many clauses, not returning a solution. (%d clauses)" % len(clauses))
            return None, None
        
        create = time.time()
        if print_timing:
            # Print 2.x
            #print 'Creating', len(clauses), 'clauses and', len(self.var_map), 'variables took', (create-start)
            # 
            # Print 3.x
            print("Creating %d clauses and %d variables took %.2f" % (len(clauses),len(self.var_map),create-start))
        ...
        if print_vars:
            for variable in solution:
                if variable > 0 and [x for (x, y) in names if x in self.num_to_var[variable]]:
                    #print self.num_to_var[variable]
                    print(self.num_to_var[variable])
    ...
    def generate_clauses(self, request, resources, print_debug=False):

        self.var_map = {}  # Map of variable names to numbers, e.g. 'droid.software.version=O' -> 1
        self.num_to_var = {}  # Map from variable numbers to names
        potential_match_map = {}
        clauses = []
        ...
        for node in request['nodes']:
            ...
            for rsrc in resource_nodes:
                #clauses += [[-self.match_var(node, rsrc), -self.match_var(node, r2)] for r2 in resource_nodes if r2 > rsrc]
                clauses += [[-self.match_var(node, rsrc), -self.match_var(node, r2)] for r2 in resource_nodes if r2 > rsrc]
            #print 'finding clauses from', node['props']['name'], node['props']
            request_clauses = self.walk_props_for_constraints(node['props'], [])  # path was node.name() but now anonymized
            if print_debug:
            ...
            for rsrc in resource_nodes:
                ...
                for clause in request_clauses:
                    grounded = self.grounded_clause(clause, rsrc)
                    if grounded and True not in grounded: # Don't add the clause if it contains True, as it is satisfiable without making a choice
                        rclauses.append([[-self.match_var(node, rsrc)] + grounded])
                ...
                for clause in request_clauses:
                    for (t, path, value) in clause:
                        v, found = self.find_or_add_var(rsrc['id'] + "." + reduce(lambda x, y: str(x) + "." + str(y), path), value)
                        if not found:  # First time we encounter the variable on a resource, check its truth
                            clauses.append([v * self.walk_resource_for_truth(rsrc['props'], path, value)])
                    # The node is matched to some resource that can match the constraints
            potential_match_map[node['props']['name']] = potential_matches
            clauses.append([self.match_var(node, potential_resource) for potential_resource in potential_matches])
        # Pairwise exclusive for resource nodes
        for r in resource_nodes:
            for t1 in request['nodes']:
                clauses += [[-self.match_var(t1, r), -self.match_var(t2, r)] for t2 in request['nodes'] if t2 > t1]
        ...
        for link in request['links']:
            ...
            for pma in potential_match_map[link_nodes[0]['props']['name']]:
                for pmb in potential_match_map[link_nodes[1]['props']['name']]:
                    if not self.link_ok(pma, pmb, link['props'], id_link_map, node_site_map):
                        # if the link won't work, don't use this pair of matches
                        clauses += [[-self.match_var(link_nodes[0], pma), -self.match_var(link_nodes[1], pmb)]]
                    else:
                        some_link_ok = True
    ...
    def walk_props_for_constraints(self, desc, path):
        res = []
        for key in desc:
            ...
            if isinstance(desc[key], list):
                ...
                res += self.walk_props_for_constraints(desc[key], path + [key])
            elif key != "name" or path != "":  # Add an equality constraint for a value, except the object name
                res.append([(1, path + [key], desc[key])])
        return res


    def grounded_clause(self, constraint_clause, resource):
        ...
        return [x for x in [self.grounded_prop(prop, resource) for prop in constraint_clause] if x != [] and x is not False]


    def grounded_prop(self, triple, resource, indent=""):
        (truth, path, value) = triple
        s = self.short_ids(resource)
        ...
        if isinstance(value, dict) and 'constraint__' in value:
            if value['constraint__'] == '?':  # choice
                ...
                for sub_value in value['value__']:
                    grounded = self.grounded_prop((truth, path, sub_value), resource, indent + "  ")
                    if grounded is True:
                        return True
    ...
    def follow_path(self, path, d):
        if path == []:
            return d
        elif path[0] in d:
            return self.follow_path(path[1:], d[path[0]])
    ...
    def walk_resource_for_truth(self, d, path, value):
        ...
        if not path or path[0] not in d:
            ...
            return self.walk_resource_for_truth(d[path[0]], path[1:], value)
        else:
            return -1


    # Define a variable stating that this resource satisfies this constraint
    def resource_constraint_var(self, resource, path, value):
        if isinstance(value, dict):
            #print 'dict is', value
            print("dict is %s" % value)
        else:
            return self.find_or_add_var(resource['id'] + path, value)  # Equality variable


    def link_ok(self, n1, n2, link_props, id_link_map, node_site_map):
    ...
    def match_var(self, query_node, resource_node):
        v, found = self.find_or_add_var(query_node['id'], resource_node['id'], '_matched_to_')
        return v


    # Return the variable that says this path should have this value. Create if necessary.
    def find_or_add_var(self, path, value, relation="="):
        var_name = path + relation + str(value)
        found = True
        if var_name not in self.var_map:
            found = False
            self.var_map[var_name] = len(self.var_map) + 1
            self.num_to_var[self.var_map[var_name]] = var_name
        return self.var_map[var_name], found


    # Functions that help manipulate sets of solutions.
    def count_solutions(self, request, resources):
        clauses = self.generate_clauses(request, resources)
    ...
    def sub_enclave_solution(self, request, resources):
        sub_solutions = []
        for res in resources:
            sublist = list(resources)
            sublist.remove(res)
            names, solution = self.solve(request, sublist)
    ...
    def gather_properties(self, enclaves):
        v = dict()
        v['endpoints'] = dict()
        for enclave in enclaves:
            for n in enclave['nodes']:
                if 'props' in n:
                    self.add_to_properties(n['props'], v)
                if 'endpoints' in n:
                    for e in n['endpoints']:
                        if 'props' in e:
                           self.add_to_properties(e['props'], v['endpoints'])
        return v


    def add_to_properties(self, data, val_dict, indent=0):
        for prop in data:
            ...
            if type(node_value) is list:
                ...
                if prop not in val_dict:
                    val_dict[prop] = dict()
                self.add_to_properties(node_value, val_dict[prop], indent + 2)
            else:
                if prop not in val_dict:
                    val_dict[prop] = set()
    ...
    def short_ids(self,orig_site):
        site = copy.deepcopy(orig_site)
        self.rec_shorten_ids(site)
        return site


    def rec_shorten_ids(self,d):
        for k in d:
            if k in ['id', 'nic']:
                if d[k] not in self.ids:
                    self.ids[d[k]] = len(self.ids)
                d[k] = self.ids[d[k]]
            elif isinstance(d[k], dict):
                self.rec_shorten_ids(d[k])
            elif isinstance(d[k], list):
                for v in d[k]:
                    if isinstance(v, dict):
                        self.rec_shorten_ids(v)
...
if __name__ == "__main__":
    ...
    mySolver = requestSolver()
    mySolver.trySubsets(small_world_json, resources, resource_names=['EMU', 'Cellular', 'IoT'])
