...
# Use loop variable $k here instead of $j in previous loop. This is to test if translator can handle this
k=1
while [ $k -le $3 ] ; do
      echo "Stopping attack on attacker$k"
      ssh -o StrictHostKeyChecking=no  attacker$k.$EXP.frade "sudo pkill -9 python3"
      k=$(($k+1))
done
sleep $INT
